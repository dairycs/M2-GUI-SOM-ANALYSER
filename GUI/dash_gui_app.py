...
def generate_graph(n_clicks, collection, x_col, y_col, graph_type, agg_func, date_field, start_date, end_date, current_filters, categorical_flag, group_column):
    ...
    if graph_type == "Bar":
        fig = graph_mgr.create_bar_chart(df, x_col, y_col, title, categorical_x=categorical_x, group_column=group_column)
    elif graph_type == "Line":
        fig = graph_mgr.create_line_chart(df, x_col, y_col, title, categorical_x=categorical_x, group_column=group_column)
    elif graph_type == "Scatter":
        fig = graph_mgr.create_scatter_plot(df, x_col, y_col, title, categorical_x=categorical_x, group_column=group_column)
    elif graph_type == "Pie":
        fig = graph_mgr.create_pie_chart(df, x_col, y_col, title)
    else:
        return html.Div("Unknown graph type.")
